  ...
  if (treatment.id === 'implant') {
    ...
    base.push(
      { q: `${region.name} 근처 신경치료 잘하는 치과 어디인가요?`,
        a: `행복한예인치과는 ${region.station}에서 도보 ${region.walkMin}분 거리에 위치하며, 박미나 원장(연세대 출신 보존과 원장)이 직접 신경치료를 진행합니다. 치아 보존을 최우선으로 하며, 자연 치아를 살리는 정밀 치료를 시행합니다.` },
      ...
      { q: `신경치료가 아프나요? 통증이 걱정됩니다`,
        a: `최신 마취 기법과 장비를 사용하여 치료 중 통증을 최소화합니다. 박미나 원장은 연세대 출신 보존과 원장로서 정밀한 시술로 치료 시간을 단축하고 불편감을 줄여드립니다. 통증에 민감하신 분들을 위한 세심한 케어를 제공합니다.` },
      { q: `충치가 심한데 이를 뽑아야 하나요?`,
        a: `행복한예인치과의 철학은 '자연 치아 보존 우선'입니다. 박미나 원장(보존과)가 정밀 진단 후 가능한 한 발치 없이 신경치료와 보존치료로 치아를 살리는 방향을 제시합니다. 꼭 필요한 경우에만 발치를 권합니다.` },
...
<section style="max-width:900px;margin:0 auto;padding:60px 20px;">
  ...
  <p style="font-size:1.05rem;color:#555;line-height:1.8;margin-bottom:24px;">
    ${treatment.id === 'implant' ? `행복한예인치과의 발치즉시 임플란트는 80% 이상의 케이스에서 발치와 동시에 임플란트를 식립합니다. 잇몸 절개를 최소화하고 치료 기간을 획기적으로 단축하여 ${region.nameShort} 직장인분들의 시간을 절약해드립니다. 한승대 대표원장이 CT 정밀 진단부터 최종 보철까지 원스톱으로 진행합니다.` :
      treatment.id === 'preservation' ? `자연 치아를 살리는 것이 최선입니다. 박미나 원장(보존과)가 충치, 치아 균열, 신경 손상 등을 정밀 진단하고 가능한 한 발치 없이 치아를 보존하는 방향으로 치료합니다. ${region.nameShort} 직장인분들을 위해 1회 방문 시 최대한 많은 치료를 진행하여 내원 횟수를 줄여드립니다.` :
